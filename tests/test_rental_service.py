...
def test_get_all_rentals(rental_service):
...
def test_find_by_property_type(rental_service):
...
def test_update_rental(rental_service):
    """Test updating a rental."""

    success, updated_rental = rental_service.update_rental(
        1, {"name": "Modern Loft"})

    assert success is True
    assert updated_rental["name"] == "Modern Loft"
    assert rental_service.get_all_rentals()[0]["name"] == "Modern Loft"


def test_delete_rental(rental_service):
    """Test deleting a rental."""

    success = rental_service.delete_rental(1)
